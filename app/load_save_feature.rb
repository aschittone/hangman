...
#   attributes = {games_won: games_won, games_lost: games_lost, user_name: user_name, letters_guessed: letters_guessed,random_word: random_word, correct_letters: correct_letters, amount_wrong_turns: amount_wrong_turns, spaces: spaces}
#   exsisting_memory = JSON.parse(IO.read('../config/memory.json'))
#   memory = File.write('../config/memory.json', exsisting_memory + attributes.to_json)
